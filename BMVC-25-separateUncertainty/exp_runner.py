...
from models.uncertain_poses import UncertainLearnPose
from models.uncertain_iou_loss import UncertaintyAwareIoULoss
...
class Runner:
    def __init__(self, conf_path, case='CASE_NAME', is_continue=False, latest_ckpt=None):
        ...
        conf_text = conf_text.replace('UNCERTAINTY_WEIGHT', str(args.uncertainty_weight))
        conf_text = conf_text.replace('IOU_WEIGHT', str(args.iou_weight))
        conf_text = conf_text.replace('ITERATIONS', str(args.iters))
        conf_text = conf_text.replace('BATCH_SIZE_COUNT', str(args.batch_size))
        f.close()
        ...
        self.uncertainty_warmup_iters = self.conf.get_int('model.uncertainty.warmup_iters', default=10000)
        self.uncertainty_weight = self.conf.get_float('model.uncertainty.weight', default=0.01)
        self.uncertainty_rampup_iters = self.conf.get_int('model.uncertainty.rampup_iters', default=20000)
        ...
        if self.learnable:
            ...
            self.use_uncertainty = self.conf.get_bool('model.pose.use_uncertainty', default=False)
            self.use_uncertainty_iou = self.conf.get_bool('model.iou.use_uncertainty', default=False)

            if self.use_uncertainty:
                # Extract the pose configuration parameters
                pose_config = dict(self.conf['model.pose'])
                # Remove 'use_uncertainty' from the config as it's only used for deciding which class to instantiate
                pose_config.pop('use_uncertainty', False)
                # Get init_uncertainty if it exists (or use default)
                init_uncertainty = pose_config.pop('init_uncertainty', 0.01) if 'init_uncertainty' in pose_config else 0.01

                # Now instantiate UncertainLearnPose without passing 'use_uncertainty'
                self.pose_param_net = UncertainLearnPose(
                    self.dataset.n_images, 
                    **pose_config,  # Now without use_uncertainty
                    init_c2w=init_poses,
                ...
                self.pose_param_net = LearnPose(
                    self.dataset.n_images, 
                    **self.conf['model.pose'], 
                    init_c2w=init_poses
                ).to(self.device)

            if self.use_uncertainty and self.dataset.with_confidence:
                # Scale initial uncertainty based on confidence scores
                confidence = torch.tensor(self.dataset.init_confidence, device=self.device)
                confidence = torch.clamp(confidence, 0.1, 1.0)  # Avoid division by zero
                
                # Set uncertainty inversely proportional to confidence
                # Higher confidence = lower uncertainty
                with torch.no_grad():
                    uncertainty_scale = 1.0 / confidence
                    uncertainty_scale = uncertainty_scale / uncertainty_scale.mean()  # Normalize
                    
                    # Apply to both rotation and translation uncertainty
                    for i in range(self.dataset.n_images):
                        self.pose_param_net.log_r_uncertainty[i] += torch.log(uncertainty_scale[i])
                        self.pose_param_net.log_t_uncertainty[i] += torch.log(uncertainty_scale[i])
    ...
    def compute_uncertainty_loss(self, img_idx1, img_idx2=None):
        """
        Compute uncertainty regularization loss
        """
        if not self.use_uncertainty or self.iter_step < self.uncertainty_warmup_iters:
            return 0.0
        
        # Compute ramp-up factor for gradual uncertainty effect
        uncertainty_factor = min(1.0, (self.iter_step - self.uncertainty_warmup_iters) / 
                            self.uncertainty_rampup_iters)
        
        # Process first camera
        r1_var = self.pose_param_net.log_r_uncertainty[img_idx1].exp()
        t1_var = self.pose_param_net.log_t_uncertainty[img_idx1].exp()
        confidence1 = torch.tensor(self.dataset.get_confidence(img_idx1), device=self.device)
        
        # Target uncertainty is inverse of confidence (higher confidence -> lower uncertainty)
        target1 = 1.0 - confidence1
        uncertainty_penalty1 = torch.mean(((r1_var.mean() + t1_var.mean())/2 - target1).abs())
        
        # Process second camera if provided
        if img_idx2 is not None:
            r2_var = self.pose_param_net.log_r_uncertainty[img_idx2].exp()
            t2_var = self.pose_param_net.log_t_uncertainty[img_idx2].exp()
            confidence2 = torch.tensor(self.dataset.get_confidence(img_idx2), device=self.device)
            target2 = 1.0 - confidence2
            uncertainty_penalty2 = torch.mean(((r2_var.mean() + t2_var.mean())/2 - target2).abs())
            uncertainty_penalty = (uncertainty_penalty1 + uncertainty_penalty2) / 2.0
        else:
            uncertainty_penalty = uncertainty_penalty1
        
        # Apply weight and ramp-up factor
        uncertainty_loss = uncertainty_penalty * self.uncertainty_weight * uncertainty_factor
    ...
    def train(self):
        ...
        n_samples = self.conf.get_int('model.iou.n_samples', default=8)
        ...
        for iter_i in tqdm(range(res_step), ncols=80):
            ...
            if balancing:
                ...
                # Add uncertainty loss for balancing case
                if self.use_uncertainty:
                    uncertainty_loss = self.compute_uncertainty_loss(img_idx)
                    if uncertainty_loss != 0.0:
                        loss = loss + uncertainty_loss
                        # Log uncertainty loss
                        self.writer.add_scalar('Loss/uncertainty_loss', uncertainty_loss.item(), self.iter_step)
                ...
                if isinstance(iou_loss, int):
                    ...
                    # Add uncertainty loss for non-balancing case
                    if self.use_uncertainty:
                        uncertainty_loss = self.compute_uncertainty_loss(img_idx1, img_idx2)
                        if uncertainty_loss != 0.0:
                            sumloss = sumloss + uncertainty_loss
                            # Log uncertainty loss
                            self.writer.add_scalar('Loss/uncertainty_loss', uncertainty_loss.item(), self.iter_step)
            ...
            if self.learnable:
                ...
                if self.use_uncertainty:
                    # Get average uncertainty for all cameras
                    avg_uncertainty = self.pose_param_net.get_uncertainty_magnitude()
                    
                    # Log uncertainty metrics
                    self.writer.add_scalar('Statistics/avg_pose_uncertainty', avg_uncertainty.item(), self.iter_step)
                    
                    # Apply adaptive learning rates
                    for g in self.optimizer_pose.param_groups:
                        g['lr'] = self.pose_lr / (1 + avg_uncertainty.item() * 5.0)  # Reduce LR when uncertain
...
if __name__ == '__main__':
    ...
    parser.add_argument('--uncertainty_weight', type=float, default=0.01,
                        help='Weight for the uncertainty term')
    parser.add_argument('--iou_weight',        type=float, default=0.2,
                        help='Weight for the IoU term')
