...
class MatchingDataset(Dataset):
    def __init__(self, conf, exp_dir=''):
        ...
        self.db_file = self.conf['colmap_db'] #os.path.join(self.conf['data_dir'], self.conf['colmap_db'])
    ...
    def get_confidence(self, img_idx):
        """
        Get confidence score for an image index
        """
        if self.init_confidence is None:
            # If no confidence has been initialized, calculate it
            confidence = self.matching_cnt / self.image_cnt
            confidence[np.isnan(confidence)] = 0
            confidence = (confidence - np.min(confidence)) / (np.max(confidence) - np.min(confidence) + 1e-5)
            return confidence[img_idx]
        else:
            # Return from pre-computed confidence
            return self.init_confidence[img_idx]
    ...
    def gen_coarse_image_ada_down(self, sigma):
        if sigma <= 1:
            self.coarse_images = self.images.to(self.device)  # Move to GPU
            ...
            self.coarse_images = torch.from_numpy(np.stack(self.coarse_images).astype(np.float32)).to(self.device)  # Move to GPU
    ...
    def gen_rays_from_colmap(self, pair_idx, batch_size):
        ...
        random_pt_x1 = torch.randint(low=0, high=self.W, size=[n_random]).to(self.device)  # Move to GPU
        random_pt_y1 = torch.randint(low=0, high=self.H, size=[n_random]).to(self.device)  # Move to GPU
        random_pt_x2 = torch.randint(low=0, high=self.W, size=[n_random]).to(self.device)  # Move to GPU
        random_pt_y2 = torch.randint(low=0, high=self.H, size=[n_random]).to(self.device)  # Move to GPU
        ...
        if n_match > 0:
            ...
            match_pt2 = torch.from_numpy(keypoints2[match[:, 1]]).to(self.device)  # Move to GPU
        ...
        # Now everything should be on GPU
        ...
        if self.learnable:
            ...
            p1 = torch.stack([pixels_x1, pixels_y1, torch.ones_like(pixels_y1)], dim=-1).float()  # Already on GPU
            p1 = torch.matmul(intrinsic_inv[None, :3, :3], p1[:, :, None]).squeeze()
            rays_v1 = p1 / torch.linalg.norm(p1, ord=2, dim=-1, keepdim=True)
            rays_v1 = torch.matmul(pose1[None, :3, :3], rays_v1[:, :, None]).squeeze()
            rays_o1 = pose1[None, :3, 3].expand(rays_v1.shape)
        else:
            p1 = torch.stack([pixels_x1, pixels_y1, torch.ones_like(pixels_y1)], dim=-1).float()  # Already on GPU
            p1 = torch.matmul(self.intrinsics_all_inv[img_idx1, None, :3, :3], p1[:, :, None]).squeeze()
            rays_v1 = p1 / torch.linalg.norm(p1, ord=2, dim=-1, keepdim=True)
            rays_v1 = torch.matmul(self.pose_all[img_idx1, None, :3, :3], rays_v1[:, :, None]).squeeze()
            rays_o1 = self.pose_all[img_idx1, None, :3, 3].expand(rays_v1.shape)
        ...
        if self.learnable:
            ...
            intrinsic_inv = torch.inverse(self.intrin_net())
            p2 = torch.stack([pixels_x2, pixels_y2, torch.ones_like(pixels_y2)], dim=-1).float()  # Already on GPU
            p2 = torch.matmul(intrinsic_inv[None, :3, :3], p2[:, :, None]).squeeze()
            rays_v2 = p2 / torch.linalg.norm(p2, ord=2, dim=-1, keepdim=True)
            rays_v2 = torch.matmul(pose2[None, :3, :3], rays_v2[:, :, None]).squeeze()
            rays_o2 = pose2[None, :3, 3].expand(rays_v2.shape)
        else:
            p2 = torch.stack([pixels_x2, pixels_y2, torch.ones_like(pixels_y2)], dim=-1).float()  # Already on GPU
            p2 = torch.matmul(self.intrinsics_all_inv[img_idx2, None, :3, :3], p2[:, :, None]).squeeze()
            rays_v2 = p2 / torch.linalg.norm(p2, ord=2, dim=-1, keepdim=True)
            rays_v2 = torch.matmul(self.pose_all[img_idx2, None, :3, :3], rays_v2[:, :, None]).squeeze()
            rays_o2 = self.pose_all[img_idx2, None, :3, 3].expand(rays_v2.shape)

        # Remove .cpu() as everything is already on GPU
        data1 = torch.cat([rays_o1, rays_v1, color1, mask1[:, :1]], dim=-1)
        data2 = torch.cat([rays_o2, rays_v2, color2, mask2[:, :1]], dim=-1)
